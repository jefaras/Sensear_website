...
3. Keep the redesign material out of the build/tooling: add `handoff` to `exclude` in `tsconfig.json` and to your eslint ignore list. (Optionally `.gitignore` `handoff/design-refs/` — the standalone files are large because images are inlined.)
